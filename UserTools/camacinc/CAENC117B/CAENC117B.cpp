	...
	if (response[0]==0){
		...
		for (int i=1;i<6;i++){		
			if (i==1){
				...
				int bit0=((channel_stats >> 0)  & 0x01);
				int bit5=((channel_stats >> 5)  & 0x01);
				int bit6=((channel_stats >> 6)  & 0x01);
				int bit8=((channel_stats >> 8)  & 0x01);
				int bit9=((channel_stats >> 9)  & 0x01);
				int bit10=((channel_stats >> 10)  & 0x01);
				int bit11=((channel_stats >> 11)  & 0x01);
				int bit12=((channel_stats >> 12)  & 0x01);
				int bit13=((channel_stats >> 13)  & 0x01);
				int bit14=((channel_stats >> 14)  & 0x01);
				int bit15=((channel_stats >> 15)  & 0x01);
...
int CAENC117B::ReadChannelParameterValues(int slot, int channel){	
	...
	if (response[0]==0){
		...
		for (int i=1;i<18;i++){		
			if (i==1){
				channel_name[0]=ConverttoAsci(HighByte(response[i]));
				channel_name[1]=ConverttoAsci(LowByte(response[i]));
			}
			else if (i==2){
				channel_name[2]=ConverttoAsci(HighByte(response[i]));
				channel_name[3]=ConverttoAsci(LowByte(response[i]));
			}
			else if (i==3){
				channel_name[4]=ConverttoAsci(HighByte(response[i]));
				channel_name[5]=ConverttoAsci(LowByte(response[i]));
			}
			else if (i==4){
				channel_name[6]=ConverttoAsci(HighByte(response[i]));
				channel_name[7]=ConverttoAsci(LowByte(response[i]));
			}
			else if (i==5){
				channel_name[8]=ConverttoAsci(HighByte(response[i]));
				channel_name[9]=ConverttoAsci(LowByte(response[i]));
			}
			else if (i==6){
				channel_name[10]=ConverttoAsci(HighByte(response[i]));
				channel_name[11]=ConverttoAsci(LowByte(response[i]));
				int i_channel=0;
...
int GetHighByte(int large_number){

	int high_byte= large_number & 0xFF;
	return high_byte;	
}

int GetLowByte(int large_number){
